...
function solve(input,chip1,chip2){
    ...
    let filteredBots = [1]
    while (filteredBots.length > 0) {
        filteredBots = bots.filter(bot => bot.values.length == 2).map(bot => {
    ...
    return parseInt(outputs[0][0]) * parseInt(outputs[1][0]) * parseInt(outputs[2][0])

}
const test1Res = solve(fs.readFileSync(__dirname+'/10test.txt', 'utf8'),5,2);
console.log(test1Res, test1Res === 30);
